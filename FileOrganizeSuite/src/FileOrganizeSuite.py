...
def run_as_admin():
    if sys.platform == "win32":
        try:
            ...
            log_message("ERROR", f"Failed to elevate privileges: {e}")
            print("Failed to elevate privileges. Please check the log for details.")
            sys.exit(1)

def upgrade_script():
    current_directory = os.path.dirname(os.path.abspath(__file__))

# The name of the Python file you want to search for
    filename_to_search = "Upgrader.py"

# Search for the file in the current directory
    file_path = os.path.join(current_directory, filename_to_search)

# Check if the file exists
    if os.path.isfile(file_path):
    # If the file exists, open it in a new terminal
        ...
        print(f"File '{filename_to_search}' not found in {current_directory}")
